...
public abstract class CameraBridgeViewBase extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void setOrientation(boolean orientation){
        if(orientation) isPortait=true;
        else isPortait=false;
    }
    ...
    protected void AllocateCache()
    {
        ...
        //mCacheBitmap要存储相机frame旋转90°后的数据
        if(isPortait){
    ...
    protected Size calculateCameraFrameSize(List<?> supportedSizes, ListItemAccessor accessor, int surfaceWidth, int surfaceHeight) {
        ...
        if(isPortait){
            maxAllowedWidth = (mMaxWidth != MAX_UNSPECIFIED && mMaxWidth < surfaceHeight)? mMaxWidth : surfaceHeight;
            maxAllowedHeight = (mMaxHeight != MAX_UNSPECIFIED && mMaxHeight < surfaceWidth)? mMaxHeight : surfaceWidth;
        }
        else{
            maxAllowedWidth = (mMaxWidth != MAX_UNSPECIFIED && mMaxWidth < surfaceWidth)? mMaxWidth : surfaceWidth;
            maxAllowedHeight = (mMaxHeight != MAX_UNSPECIFIED && mMaxHeight < surfaceHeight)? mMaxHeight : surfaceHeight;
        }
